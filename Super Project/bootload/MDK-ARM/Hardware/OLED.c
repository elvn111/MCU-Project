...
void OLED_ShowPicture(void)
{
	...
	for(int i=7;i>=0;i--)
	{
		for(int j=0;j<128;j++)
		{
			OLED_I2C_SendByte(OLEDPicture[j][i]);
		}
	}
	OLED_I2C_Stop();
}
...
void OLED_Horizon(int X,int Y,int Z)
{
	...
	int xultimate =2000;
	int yultimate =2000;
	memset(OLEDPicture[0],0,1024);
	if((X<=xultimate&&X>=-xultimate)&&(Y<=yultimate&&Y>=-yultimate)){
		if(Z) memset(OLEDPicture[0],0xff,1024);
		else memset(OLEDPicture[0],0x00,1024);
	}
	else{ 
		for(int i=0;i<128;i++){
			if(X<=0){
				...
				if(Axis_k<0){
					...
					memset(OLEDPicture[i],0xff,8);
				}else{
					memset(OLEDPicture[i],0xff,(Axis_k/8));
					OLEDPicture[i][Axis_k/8]=0xff<<(8-(Axis_k%8));
				}
			}
			else{
				if(Y>=0){
				...
				if(Axis_k<0){
					...
					memset(OLEDPicture[i]+(Axis_k/8),0xff,8-(Axis_k/8));
					OLEDPicture[i][Axis_k/8]=0xff>>(Axis_k%8);
				}
			}
		}
	}
	//刻画准星
	OLEDPicture[63][3]=~OLEDPicture[63][3];
	OLEDPicture[63][2]=~OLEDPicture[63][2];
	OLEDPicture[64][3]=~OLEDPicture[64][3];
	OLEDPicture[64][2]=~OLEDPicture[64][2];
	for(int a=55;a<=72;a++){
		OLEDPicture[a][2]^=1;
		OLEDPicture[a][3]^=1<<7;
	}
